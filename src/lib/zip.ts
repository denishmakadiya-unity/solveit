// Minimal ZIP writer (STORE method) for bundling multiple outputs into one download.
const CRC_TABLE = (() => {
...
export async function makeZip(files: { name: string; blob: Blob }[]): Promise<Blob> {
  ...
  for (const f of files) {
    ...
    let i = 1;
    while (used.has(name)) name = f.name.replace(/(\.[^.]+)?$/, (m) => `-${i++}${m}`);
    used.add(name);
  ...
  return new Blob([...chunks, ...central, end.buffer] as BlobPart[], { type: "application/zip" });
}
